...
args.app.get('/p/:pad/:rev?/comments', function(req, res) {
    if(!apiUtils.validateApiKey({api_key: req.query.api_key}, res)) return;
      // sanitize pad id before continuing
      var padIdReceived = apiUtils.sanitizePadId(req);
      comments.getPadComments(padIdReceived, function(padID, padComments) {
        res.json({code: 0, comments: padComments});
      });
  });
