...
.on("dragleave", function(e) {
 $(this).css("border", "none");
    ...
    $inner.on("drop", function(e){
      console.log('image dropped again');
      var file;
      ...
      if (file.type.match('image.*')) {
        ...
        reader.onload = (function(theFile) {
          ...
          $.ajax({
           // url: settings.ep_copy_paste_images.upload_url, //'http://hi-map.org/stage/api/upload_ether_image',
            url: 'http://collab.icimod.org/api/upload_ether_image',
            data: {img: dataURI},
            //processData: false,
            //contentType: false,
            //datatype: 'json',
          ...
          context.ace.callWithAce(function(ace){
            ...
            ace.ace_addImage(rep, '"' + response.image.path + '"', response.image);
          }, 'img', true);
        }
          })
          // Now to insert the base64 encoded image into the pad
...
var image = {
  ...
  removeImage: function(lineNumber){
    ...
    documentAttributeManager.removeAttributeOnLine(lineNumber, 'imageHeight'); // remove img attributes 
    documentAttributeManager.removeAttributeOnLine(lineNumber, 'imageWidth'); // remove img attributes 
  },

  addImage: function(rep, src, imgAttributes){
    ...
    documentAttributeManager.setAttributeOnLine(lineNumber, 'imageHeight', imgAttributes.height); // make the line a task list
    documentAttributeManager.setAttributeOnLine(lineNumber, 'imageWidth', imgAttributes.width); // make the line a task list
  }
...
exports.aceDomLineProcessLineAttributes = function(name, context){
  ...
  if (imgType[1] && imgType[1].indexOf('data:') < 0 && imgType[1].indexOf(';base64,') < 0){
...
exports.collectContentImage = function(name, context){
  ...
  if(tname == "img") {
    if(context.node.attributes[1].value.indexOf('data:') >=0 && context.node.attributes[1].value.indexOf(';base64,') >= 0) {
      delete lineAttributes['img'];
      delete lineAttributes['imgSize'];
    } else {
      if(tname == "img"){
...
function checkIfLineContainsTableData(rep) {
    console.log(rep);
