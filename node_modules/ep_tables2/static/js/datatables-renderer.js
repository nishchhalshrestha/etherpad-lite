if (typeof (DatatablesRenderer) == 'undefined') var DatatablesRenderer = function () {
		...
		if (params != "timeslider" && element.innerHTML && element.innerHTML.indexOf("payload") != 2) return;
                ...
                if (params == "export") {
                  ...
                  return renderer.getHtmlForDocument(code, attributes);
                } else {
        ...
        dRenderer.Renderer.prototype = {
            ...
            buildTabularDataForDocument: function (tblJSONObj, tblPropsJSString) {
                var htmlTbl = "";
                var tblId = tblJSONObj.tblId;
                var tblClass = tblJSONObj.tblClass;
                var tdClass = tblJSONObj.tdClass;
                ...
                for (var j = 0, rl = rows.length; j < rl; j++) {
                    ...
                    for (var i = 0, tl = tds.length; i < tl; i++) {
                        ...
                        if (tds[i].indexOf('/r/n') != -1) {
                            ...
                            htmlTbl += "<td  name='tData' " + colVAlign + " width='" + tdWidth + "px' style='" + printViewTblTDStyles + cellStyles + " border-left:" + 
                            tblBorderWidth + "px solid " + tblBorderColor + ";" + borderTop + lastCellBorder + "' >" + cellsWithBr + 
                            "<br value='tblBreak'></td>" + delimCell;
                        } else {
                            htmlTbl += "<td name='tData' " + colVAlign + " width='" + tdWidth + "px' style='" + printViewTblTDStyles + cellStyles + lastCellBorder + " border-left:" + tblBorderWidth + "px solid " + tblBorderColor + ";" + borderTop + "' >" + tds[i] + "" + "<br value='tblBreak'></td>" + delimCell
                        }
                    }
                    var bracketAndcomma = "\"]],\"tblId\":\"1\",\"tblClass\":\"data-tables\"}";
                    //htmlTbl += "<td name='bracketAndcomma' class='  hide-el overhead' style='display:none;'>" + bracketAndcomma + "</td>";
            ...
            getHtmlForDocument: function(code, attributes) {
                var JSONCode = "";
                var html = "";
                try {
                    JSONCode = JSON.parse(code);
                    html = this.buildTabularDataForDocument(JSONCode, attributes);
                } catch (error) {}
                return html;  
            }
        };
